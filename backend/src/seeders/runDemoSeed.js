...
const { sequelize, Role, User, IncidentType, Incident, ThreatFeed, Assignment, InvestigationNote, Evidence, StatusLog, Notification, AuditLog, Resolution } = require('../models');
...
const runSeed = async () => {
  ...
  try {
    ...
    // Admin
    const admin = await User.create({ role_id: roles['admin'].role_id, full_name: 'Admin User', email: 'admin@sentinelx.io', password_hash: passwordHash, department: 'SOC' });

    // Analysts
    const analysts = [];
    for (let i = 1; i <= 5; i++) {
      analysts.push(await User.create({ role_id: roles['analyst'].role_id, full_name: `Analyst ${i}`, email: `analyst${i}@sentinelx.io`, password_hash: passwordHash, department: 'SOC Analyst' }));
    }

    // Reporters
    const reporters = [];
    for (let i = 1; i <= 20; i++) {
      reporters.push(await User.create({ role_id: roles['reporter'].role_id, full_name: `Reporter ${i}`, email: `reporter${i}@company.com`, password_hash: passwordHash, department: 'General' }));
    }
